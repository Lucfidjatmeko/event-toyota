export async function POST(request){
  try {
      ...
      const allDataEvent = await fetch (`${origin}/api/event-data`)
      const dataEvent = await allDataEvent.json()
      // const resultData = dataEvent.data.find(el => el.id == id)
      //console.log(resultData);
      
      let resultSearch = dataEvent.data.filter(el => el.title.toLowerCase().includes(searchTerm.toLowerCase()))

      console.log(resultSearch," ===> INI APA SIH HAHAHA");
